...
function RedisClient(type, keyIds, version) {
    ...
    this.value = '';
    this.client = RedisClient.getClient(RedisClient.key(type, keyIds, version));
...
RedisClient.keysPro = function(pattern) {
    let clients = RedisClient.getClients();
    let arrRedis = _.map(clients, (redis, key) => redis);
    return Promise.map(arrRedis, (redis) => {
        return redis.keysAsync(pattern);
    })
...
RedisClient.getPro = function(key) {
    let client = RedisClient.getClient(key);
...
RedisClient.prototype.xLockOrder = async function(doc, expire) {
    let self = this,
        key = RedisClient.key(self.type, self.keyIds, self.version);

    const exist = await self.client.existsAsync(key);
    if (!_.isEmpty(exist)) {
        return null;
    }
    const length = await self.client.rpushAsync(key, JSON.stringify(doc)); // the length of the list after the push operation, 1 -> 2 -> 3
    const expireTime = expire || RedisClient.expire(self.type);

    if (!_.isNil(expireTime)) {
        await self.client.expireAsync(key, expireTime);
    }
    return length;
    // const firstValue = await self.client.lrangeAsync(key, 0, 0);
